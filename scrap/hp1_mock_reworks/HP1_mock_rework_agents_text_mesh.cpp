...
// Can we use fixup_ghosting_to_shared as a means of not having neighbor linkers?
// Well, this would get rid of the need for neighbor linkers for helping with binding.


void compute_bind_left_bound_harmonic_to_sphere_z_partition(const DynamicHookeanSprings& dynamic_hookean_springs,
    const double kt,
    const stk::mesh::Field<double> &c_state_change_rate_field,
    const stk::mesh::Selector &spring_sphere_linker_selector) {
  const double inv_kt = 1.0 / kt;
  stk::mesh::for_each_entity_run(
      n_coord_field.get_mesh(), stk::topology::CONSTRAINT_RANK, spring_sphere_linker_selector,
...
void kmc_spring_left_to_doubly(const DynamicHookeanSprings& dynamic_hookean_springs const double timestep_size,
                               const stk::mesh::Field<unsigned> &c_perform_state_change_field,
                               const stk::mesh::Field<double> &c_state_change_rate_field,
                               const stk::mesh::Selector &spring_sphere_linkers) {
  // Loop over left-bound spring and decide if they bind or not
  stk::mesh::for_each_entity_run(
      c_perform_state_change_field.get_mesh(), stk::topology::ELEMENT_RANK, left_bound_spring_selector,
...
void kmc_spring_doubly_to_right(const stk::mesh::Selector &doubly_bound_spring_selector,
                                const stk::mesh::Selector &right_bound_spring_selector,
                                const stk::mesh::Selector &spring_sphere_genx_selector,
                                stk::mesh::Field<unsigned> &el_perform_state_change_field,
                                stk::mesh::Field<unsigned> &c_perform_state_change_field,
                                stk::mesh::Field<double> &el_unbinding_rates_field) {
...
void perform_state_change_dynamic_springs(const stk::mesh::Selector &left_bound_spring_selector,
                                          const stk::mesh::Selector &doubly_bound_spring_selector,
                                          const stk::mesh::Selector &spring_sphere_genx_selector,
                                          stk::mesh::Field<unsigned> &el_perform_state_change_field,
                                          stk::mesh::Field<unsigned> &c_perform_state_change_field) {
...
void check_max_overlap_with_spherical_periphery(stk::mesh::BulkData &bulk_data, const double target_sphere_radius,
                                                const std::array<double, 3> &target_sphere_center,
                                                const bool invert_target_sphere,
                                                const stk::mesh::FieldBase &n_coord_field,
                                                const stk::mesh::FieldBase &element_radius_field,
                                                const stk::mesh::Selector &spheres_selector,
...
void check_max_overlap_with_ellipsoidal_periphery_fast_approx(
    stk::mesh::BulkData &bulk_data, const double target_ellipsoid_r1, const double target_ellipsoid_r2,
    const double target_ellipsoid_r3, const std::array<double, 3> &target_ellipsoid_center,
    const bool invert_target_ellipsoid, const stk::mesh::FieldBase &n_coord_field,
    const stk::mesh::FieldBase &element_radius_field, const stk::mesh::Selector &spheres_selector,
    const double max_allowable_overlap) {
...
void compute_rpy_hydro(stk::mesh::BulkData &bulk_data, const double viscosity,
                       const stk::mesh::FieldBase &n_coord_field, const stk::mesh::FieldBase &element_radius_field,
                       const stk::mesh::FieldBase &n_force_field, const stk::mesh::FieldBase &n_velocity_field,
                       const stk::mesh::Selector &spheres_selector, const double max_allowable_overlap,
                       const double target_sphere_radius, const std::array<double, 3> &target_sphere_center,
                       const bool invert_target_sphere, const double target_ellipsoid_r1,
                       const double target_ellipsoid_r2, const double target_ellipsoid_r3,
                       const std::array<double, 3> &target_ellipsoid_center, const bool invert_target_ellipsoid,
                       const bool validate_noslip_boundary, const mundy::alens::Periphery *periphery) {
...
void compute_ellipsoidal_periphery_collision_forces_with_spheres(
    stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &spheres_selector, const double periphery_r1,
    const double periphery_r2, const double periphery_r3, const std::array<double, 3> &periphery_center,
    const mundy::math::Quaternion<double> &periphery_orientation, const double collision_spring_constant,
    const stk::mesh::Field<double> &n_coord_field, const stk::mesh::Field<double> &element_radius_field,
    const stk::mesh::Field<double> &n_force_field) {
...
void compute_ellipsoidal_periphery_collision_forces_with_spheres_fast_approx(
    stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &spheres_selector, const double periphery_r1,
    const double periphery_r2, const double periphery_r3, const std::array<double, 3> &periphery_center,
    const mundy::math::Quaternion<double> &periphery_orientation, const double collision_spring_constant,
    const stk::mesh::Field<double> &n_coord_field, const stk::mesh::Field<double> &element_radius_field,
...
void compute_spherical_periphery_collision_forces_with_spheres(
    stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &spheres_selector, const double &periphery_radius,
    const std::array<double, 3> &periphery_center, const double &collision_spring_constant,
    const stk::mesh::Field<double> &n_coord_field, const stk::mesh::Field<double> &element_radius_field,
    const stk::mesh::Field<double> &n_force_field) {
  stk::mesh::for_each_entity_run(
      *bulk_data_ptr_, stk::topology::ELEMENT_RANK, spheres_selector,
...
void compute_brownian_velocity_spheres(stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &selector,
                                       const double &timestep_size, const double &viscosity, const double &brownian_kt,
                                       const double &sphere_radius, const stk::mesh::Field<double> &n_rng_field,
...
void compute_dry_local_drag_spheres(stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &selector,
                                    const double &viscosity, const double &sphere_radius,
                                    const stk::mesh::Field<double> &n_force_field,
                                    const stk::mesh::Field<double> &n_velocity_field) {
...
void node_euler_position_integrator(stk::mesh::BulkData &bulk_data, const stk::mesh::Selector &selector,
                                    const double &timestep_size, const stk::mesh::Field<double> &old_n_coord_field,
...
int main(int argc, char **argv) {
...

  {
    ...
    auto &backbone_backbone_linker_part =
        mundy::declare_io_part(meta_data, "BACKBONE_BACKBONE_NEIGHBOR_GENX", CONSTRAINT_RANK);
    auto &hp1_h_linker_part = mundy::declare_io_part(meta_data, "HP1_H_NEIGHBOR_GENX", CONSTRAINT_RANK);
    auto &hp1_bs_linker_part = mundy::declare_io_part(meta_data, "HP1_BS_NEIGHBOR_GENX", CONSTRAINT_RANK);
    ...
    auto &c_potential_force_field = meta_data.declare_field<double>(CONSTRAINT_RANK, "POTENTIAL_FORCE");
    auto &c_state_change_rate_field = meta_data.declare_field<double>(CONSTRAINT_RANK, "STATE_CHANGE_RATE");
    auto &c_perform_state_change_field = meta_data.declare_field<unsigned>(CONSTRAINT_RANK, "PERFORM_STATE_CHANGE");
    ...
    for (size_t timestep_idx = 0; timestep_idx < num_time_steps_; timestep_idx++) {
      ...
      if (max_aabb_displacement > skin_distance) {
        ...
        if (enable_crosslinkers_) {
          mundy::generate_neighbor_linkers(hp1_selector, h_selector, el_aabb_field, el_aabb_field,
                                           hp1_h_linker_part);
        }
        if (enable_periphery_binding_) {
          mundy::generate_neighbor_linkers(hp1_selector, bs_selector, el_aabb_field, el_aabb_field,
                                           hp1_bs_linker_part);
        }
      }
