...
class FlexibleFiberHelper {
 public:
  FlexibleFiberHelper(const size_t num_nodes, const bool generate_backbone_segments, const bool generate_spheres_at_nodes,
                const bool generate_angular_springs, const size_t node_start_id, const size_t element_start_id,
                const bool distributed);

  const stk::mesh::Entity &get_segement(const size_t segment_id);
  const stk::mesh::Entity &get_sphere(const size_t sphere_id);
  const stk::mesh::Entity &get_angular_spring(const size_t angular_spring_id);
  const stk::mesh::Entity &get_node(const size_t node_id);
  const std::vector<size_t> &get_locally_owned_segment_ids();
  const std::vector<size_t> &get_locally_owned_sphere_ids();
  const std::vector<size_t> &get_locally_owned_angular_spring_ids();
  const std::vector<size_t> &get_locally_owned_or_shared_node_ids();

  void declare(const stk::mesh::BulkData &bulk_data);

 private:
  const size_t num_nodes_;
  const bool generate_backbone_segments_;
  const bool generate_spheres_at_nodes_;
  const bool generate_angular_springs_;
  const size_t node_start_id_;
  const size_t element_start_id_;
  const bool distributed_;

  std::vector<size_t> locally_owned_segment_ids_;
  std::vector<size_t> locally_owned_sphere_ids_;
  std::vector<size_t> locally_owned_angular_spring_ids_;
  std::vector<size_t> locally_owned_or_shared_node_ids_;
};

class LeftBoundHP1Helper {
 public:
  HP1Helper(const std::vector<size_t> &)
...
int main(int argc, char **argv) {
...

  {
    ...
    if (!restart_performed_) {
      ChromatinHelper(num_chromosomes, num_chromatin_repeats, num_euchromatin_per_repeat,
                      num_heterochromatin_per_repeat, enable_backbone_collision, enable_backbone_springs);
      DeclareHP1Helper(DeclareChromatinHelper);

      mundy::declare_chromatin_and_hp1(num_chromosomes, num_chromatin_repeats, num_euchromatin_per_repeat,
                                       num_heterochromatin_per_repeat, enable_backbone_collision,
                                       enable_backbone_springs, enable_crosslinkers);

      mundy::declare_entities(declare_entity_info, requested_nodes)
    ...
    for (timestep_index_ = 0; timestep_index_ < num_time_steps_; timestep_index_++) {
      ...
      mundy::deep_copy(node_velocity_field,  //
                       std::array<double, 3>{0.0, 0.0, 0.0});
      mundy::deep_copy(node_force_field,  //
                       std::array<double, 3>{0.0, 0.0, 0.0});
      mundy::deep_copy(element_binding_rates_field,  //
                       std::array<double, 2>{0.0, 0.0});
      mundy::deep_copy(element_unbinding_rates_field,  //
                       std::array<double, 2>{0.0, 0.0});
      mundy::deep_copy(element_perform_state_change_field,  //
                       std::array<unsigned, 1>{0u});
      mundy::deep_copy(constraint_perform_state_change_field,  //
                       std::array<unsigned, 1>{0u});
      mundy::deep_copy(constraint_state_change_rate_field,  //
                       std::array<double, 1>{0.0});
      mundy::deep_copy(constraint_potential_force_field,  //
                       std::array<double, 3>{0.0, 0.0, 0.0});
      ...
      mundy::compute_aabb(segs, skin_distance, element_aabb_field);
      mundy::compute_aabb(spheres, skin_distance, element_aabb_field);
      const double alpha = 1.0;
      const double beta = 1.0;
      mundy::compute_aabb_displacements(element_old_aabb_field, element_aabb_field, element_aabb_displacement_field,
                                        alpha, beta);
      ...
      mundy::compute_max_aabb_displacement(element_aabb_displacement_field, max_aabb_displacement);
      ...
      if (max_aabb_displacement > skin_distance) {
        ...
        mundy::deep_copy(element_aabb_displacement_field,  //
        ...
        if (enable_backbone_collision_ || enable_crosslinkers_ || enable_periphery_binding_) {
          ...
          const auto neighbor_selector = backbone_backbone_neighbor_genx_part | spring_sphere_neighbor_genx_part;
          mundy::destroy_distant_neighbors(neighbor_selector, max_allowed_distance, element_aabb_field);
        }
        ...
        if (enable_backbone_collision_) {
          ...
          mundy::generate_neighbor_linkers(backbone_segment, backbone_segments, element_aabb_field,
                                           element_aabb_field, backbone_backbone_neighbor_genx_part);
          mundy::destroy_bound_neighbors(backbone_backbone_neighbor_genx_selector);
        }
        if (enable_crosslinkers_) {
          mundy::generate_neighbor_linkers(hp1_selector, h_selector, element_aabb_field, element_aabb_field,
                                           hp1_h_neighbor_genx_part);
        }
        if (enable_periphery_binding_) {
          mundy::generate_neighbor_linkers(hp1_selector, bs_selector, element_aabb_field, element_aabb_field,
                                           hp1_bs_neighbor_genx_part);
        }
      }

      if (enable_crosslinkers_) {
        mundy::compute_state_change_rate_attach_left_bound_attach_to_node(
            binding_kt, dynamic_springs, dynamic_springs_to_node_linkers, constraint_state_change_rate_field);
        mundy::kmc_decide_state_change_left_bound_attach_to_node(
            timestep_size, dynamic_springs, dynamic_springs_to_node_linkers, constraint_state_change_rate_field,
            constraint_perform_state_change_field);
        mundy::kmc_decide_state_change_detach_doubly_bound_from_node(timestep_size, dynamic_springs);
        mundy::perform_state_change(dynamic_springs, dynamic_springs_to_node_linkers,
                                    constraint_perform_state_change_field);
      }
      ...
      if (enable_backbone_springs_) {
        mundy::compute_hookean_spring_forces(backbone_springs, node_force_field);
      }

      if (enable_crosslinkers_) {
        // Select only active springs in the system. Aka, not left bound.
        mundy::compute_hookean_spring_forces(doubly_bound_dynamic_springs, node_force_field);
      }
      ...
      if (enable_chromatin_brownian_motion_) {
        mundy::compute_brownian_velocity(timestep_size, viscosity, brownian_kt, spheres, node_rng_counter_field,
                                         node_force_field, node_velocity_field);
      }

      if (enable_backbone_n_body_hydrodynamics_) {
        const bool validate_noslip_boundary = true;
        mundy::compute_rpy_hydrodynamics(viscosity, hydrodynamic_spheres, no_slip_periphery_evaluator, node_force_field,
                                         node_velocity_field, validate_noslip_boundary);
      } else {
        mundy::compute_dry_local_drag(viscosity, spheres, node_force_field, node_velocity_field);
      }
      ...
      mundy::integrate_positions_node_euler(timestep_size, hydrodynamic_spheres, node_velocity_field);
    }
