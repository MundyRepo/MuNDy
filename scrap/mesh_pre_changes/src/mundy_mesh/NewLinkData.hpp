...
#ifndef MUNDY_MESH_NEW_LINKDATA_HPP_
#define MUNDY_MESH_NEW_LINKDATA_HPP_
...
#include <mundy_mesh/impl/LinkDataCRSManager.hpp>  // for mundy::mesh::impl::LinkDataCRSManager

namespace mundy {

namespace mesh {

// Foreward decarations 
class NewNgpLinkDataBase;

namespace impl {
NewNgpLinkDataBase* get_ngp_link_data(const NewLinkData & link_data); 
}
...
class NewLinkData {
 public:
  ...
  NewLinkData(const NewLinkData &) = default;
  NewLinkData(NewLinkData &&) = default;
  NewLinkData &operator=(const NewLinkData &) = default;
  NewLinkData &operator=(NewLinkData &&) = default;
  ...
  NewLinkData(BulkData &bulk_data, NewLinkMetaData &link_meta_data)  // We do NOT take ownership of the LinkMetaData
      : bulk_data_ptr_(&bulk_data), mesh_meta_data_ptr_(&bulk_data.mesh_meta_data()), link_meta_data_ptr_(&link_meta_data), crs_manager_(this) {
  }

  /// \brief Destructor.
  virtual ~NewLinkData() = default;
  //@}
  ...
  ///
  /// Safety: This view is always safe to use and the reference is valid as long as the NewLinkData is valid, which
  ...
  /// is up to date without performing an update, call sync_crs(mem_space).
  ///
  ...
  const NgpCRSPartitionViewT<NgpMemSpace> &get_all_crs_partitions(const NgpMemSpace &mem_space = NgpMemSpace{}) {
    return crs_manager_.get_all_crs_partitions(mem_space);
  }
  ...
  /// mundy will make is to never delete a partition outside of a modification cycle.
  template <typename NgpMemSpace = stk::ngp::HostMemSpace>
    requires(Kokkos::is_memory_space_v<NgpMemSpace>)
  const NgpCRSPartitionViewT<NgpMemSpace> &get_or_create_crs_partitions(
    ...
    return crs_manager_.get_or_create_crs_partitions(selector, mem_space);
  }
  ...
  bool is_crs_up_to_date(const stk::mesh::Selector &selector,
    ...
    return crs_manager_.is_crs_up_to_date(selector, mem_space);
  }
  ...
  bool is_crs_up_to_date(const NgpMemSpace &mem_space = NgpMemSpace{}) {
    return crs_manager_.is_crs_up_to_date(mem_space);
  }
  ...
  template <typename NgpMemSpace = stk::ngp::HostMemSpace>
    requires(Kokkos::is_memory_space_v<NgpMemSpace>)
  void is_crs_up_to_date(const stk::mesh::Selector &selector, const NgpMemSpace &mem_space = NgpMemSpace{}) {
    crs_manager_.update_crs_from_coo(selector, mem_space);
  }
  ...
  void update_crs_from_coo(const NgpMemSpace &mem_space = NgpMemSpace{}) {
    crs_manager_.update_crs_from_coo(mem_space);
  }
  ...
  void check_crs_coo_consistency(const stk::mesh::Selector &selector, const NgpMemSpace &mem_space = NgpMemSpace{}) {
    crs_manager_.check_crs_coo_consistency(selector, mem_space);
  }
  ...
  void check_crs_coo_consistency(const NgpMemSpace &mem_space = NgpMemSpace{}) {
    crs_manager_.check_crs_coo_consistency(mem_space);
  }
  //@}
  ...
  const NewNgpLinkRequests &get_or_create_link_requests(
      const stk::mesh::PartVector &link_parts, unsigned requested_dimensionality = 0,
      unsigned requested_capacity = 0) const {  // MOD MARK: NgpLinkData should call LinkData.
  ...
  void modify_coo_on_host() {
    impl::get_linked_entities_field(link_meta_data).modify_on_host();
    link_meta_data().linked_entities_crs_field().modify_on_host();
    link_meta_data().linked_entity_ids_field().modify_on_host();
    link_meta_data().linked_entity_ranks_field().modify_on_host();
    link_meta_data().linked_entity_bucket_ids_field().modify_on_host();
    link_meta_data().linked_entity_bucket_ords_field().modify_on_host();
    link_meta_data().link_crs_needs_updated_field().modify_on_host();
    link_meta_data().link_marked_for_destruction_field().modify_on_host();
  }

  void modify_coo_on_device() {
    impl::get_linked_entities_field(link_meta_data).modify_on_device();
    link_meta_data().linked_entities_crs_field().modify_on_device();
    link_meta_data().linked_entity_ids_field().modify_on_device();
    link_meta_data().linked_entity_ranks_field().modify_on_device();
    link_meta_data().linked_entity_bucket_ids_field().modify_on_device();
    link_meta_data().linked_entity_bucket_ords_field().modify_on_device();
    link_meta_data().link_crs_needs_updated_field().modify_on_device();
    link_meta_data().link_marked_for_destruction_field().modify_on_device();
  }

  void sync_coo_to_host() {
    impl::get_linked_entities_field(link_meta_data).sync_to_host();
    link_meta_data().linked_entities_crs_field().sync_to_host();
    link_meta_data().linked_entity_ids_field().sync_to_host();
    link_meta_data().linked_entity_ranks_field().sync_to_host();
    link_meta_data().linked_entity_bucket_ids_field().sync_to_host();
    link_meta_data().linked_entity_bucket_ords_field().sync_to_host();
    link_meta_data().link_crs_needs_updated_field().sync_to_host();
    link_meta_data().link_marked_for_destruction_field().sync_to_host();
  }

  void sync_coo_to_device() {
    impl::get_linked_entities_field(link_meta_data).sync_to_device();
    link_meta_data().linked_entities_crs_field().sync_to_device();
    link_meta_data().linked_entity_ids_field().sync_to_device();
    link_meta_data().linked_entity_ranks_field().sync_to_device();
    link_meta_data().linked_entity_bucket_ids_field().sync_to_device();
    link_meta_data().linked_entity_bucket_ords_field().sync_to_device();
    link_meta_data().link_crs_needs_updated_field().sync_to_device();
    link_meta_data().link_marked_for_destruction_field().sync_to_device();
  }

  /// \brief Synchronize the CRS connectivity with the most up-to-date memory space.
  template <typename NgpMemSpace>
    requires(Kokkos::is_memory_space_v<NgpMemSpace>)
  void sync_crs(const NgpMemSpace &mem_space) {
    crs_manager_.sync(mem_space);
  }
  void sync_crs_to_host() {
    crs_manager_.sync(stk::ngp::HostMemSpace{});
  }
  void sync_crs_to_device() {  // Default "device" of stk::ngp::MemSpace
    crs_manager_.sync(stk::ngp::MemSpace{});
  }
  //@}
 ...
 protected:
  ...
  inline unsigned get_linker_dimensionality(const stk::mesh::Entity &linker) const {
    ...
    return stk::mesh::field_scalars_per_entity(impl::get_linked_entities_field(link_meta_data), linker);
  }
  ...
  inline unsigned get_linker_dimensionality(const stk::mesh::PartVector &parts) const {
  ...
  inline unsigned get_linker_dimensionality(const PartitionKey &partition_key) const {
    ...
    return get_linker_dimensionality(parts);
  }
  ...
  void destroy_marked_links() {  // MOD MARK: NgpLinkData should call LinkData
    auto &link_marked_for_destruction_field = link_meta_data().link_marked_for_destruction_field();
 ...
 private:
  ...
  void set_ngp_link_data(NewNgpLinkDataBase* ngp_link_data) const { 
    ngp_link_data_ = ngp_link_data; 
  }
  
  NewNgpLinkDataBase * get_ngp_link_data() const { 
    return ngp_link_data_; 
  }
  //@}
  ...
  friend NewNgpLinkDataBase* impl::get_ngp_link_data(const NewLinkData & link_data); 
  //@}
  ...
  impl::LinkDataCRSManager<NewLinkData> crs_manager_;
  mutable NewNgpLinkDataBase *ngp_link_data_;
...
inline NewNgpLinkDataBase* get_ngp_link_data(const NewLinkData & link_data) {
  return link_data.get_ngp_link_data();
}

inline void set_ngp_link_data(const NewLinkData & link_data, NewNgpLinkDataBase * ngp_link_data) {
  link_data.set_ngp_link_data(ngp_link_data);
}

}  // namespace impl

/// \brief Declare a new NewLinkData object.
///
/// Note, this is the de-facto constructor for NewLinkData. In the future, we will have it return a reference to
/// the constructed NewLinkData object by tying its lifetime to the BulkData object.
///
/// \param bulk_data [in] The bulk data manager we extend.
/// \param link_meta_data [in] Our meta data manager. Must be persistant with a lifetime at least as long as the
///   generated NewLinkData.
/// \return A new NewLinkData object.
NewLinkData declare_link_data(BulkData &bulk_data, NewLinkMetaData &link_meta_data) {
  // TODO(palmerb4): Store a map of NewLinkData objects in the BulkData object using an attribute
  //  this way, we can tie the lifetime of the NewLinkData object to the BulkData object and return
  //  a reference here.
  return NewLinkData(bulk_data, link_meta_data);
}
...
#endif  // MUNDY_MESH_NEW_LINKDATA_HPP_
