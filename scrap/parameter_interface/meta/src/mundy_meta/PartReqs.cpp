...
bool PartReqs::has_entity_induction() const {
  if (has_master_part_reqs_) {
    ...
    MUNDY_THROW_REQUIRE(has_entity_induction_is_set_, std::logic_error,
                        std::string("PartReqs: Attempting to access the entity induction requirement even though "
                                    "entity induction is unconstrained."));
...
stk::mesh::Part &PartReqs::declare_part_on_mesh(mundy::mesh::MetaData *const meta_data_ptr) const {
  ...
  std::cout << "Part with name: " << this->get_part_name()
            << (this->constrains_part_induction() ? " has entity induction." : " does not have entity induction.")
            << " and force_no_induce is " << arg_force_no_induce << std::endl;
  ...
  if (this->constrains_part_topology()) {
    part_ptr = &meta_data_ptr->declare_part_with_topology(this->get_part_name(), this->get_part_topology(),
                                                          arg_force_no_induce);
    MUNDY_THROW_REQUIRE(part_ptr->force_no_induce() == arg_force_no_induce, std::runtime_error,
                        "PartReqs: The force_no_induce flag was not set correctly.");
