...
class NgpLinkDataCRSManagerT {
 public:
  ...
  NgpLinkDataCRSManagerT(const NgpLinkDataCRSManagerT &) = default;
  NgpLinkDataCRSManagerT(NgpLinkDataCRSManagerT &&) = default;
  NgpLinkDataCRSManagerT &operator=(const NgpLinkDataCRSManagerT &) = default;
  NgpLinkDataCRSManagerT &operator=(NgpLinkDataCRSManagerT &&) = default;

  /// \brief Canonical constructor.
  NgpLinkDataCRSManagerT(NewNgpLinkDataT<NgpMemSpace> *link_data_ptr)
  ...
  /// \brief Destructor.
  virtual ~NgpLinkDataCRSManagerT() = default;
  //@}
  ...
  done_checking_crs_buckets:
    ...
    if (crs_buckets_up_to_date) {  // No need to perform the second check if the first fails.
      ...
      int link_needs_updated_count = ::mundy::mesh::field_sum<int>(
          link_data_ptr_->link_meta_data().link_crs_needs_updated_field(), link_subset_selector, stk::ngp::ExecSpace());
...
// If in debug, check consistency
  ...
  void update_stk_link_bucket_to_partition_id_map() {
    ...
    const stk::mesh::BucketVector &all_link_buckets =
        link_data_ptr_->bulk_data().get_buckets(link_data_ptr_->link_meta_data().link_rank(), all_selector);
    ...
    Kokkos::parallel_for("flag_dirty_linked_buckets_of_modified_links",
        team_policy, KOKKOS_LAMBDA(const TeamHandleType &team) {
          ...
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0u, num_links), [&](const int &i) {
            ...
            if (link_data.get_link_crs_needs_updated(link_index)) {  // MOD_MARK
              // Loop over the linked entities of this link
              for (unsigned d = 0; d < dimensionality; ++d) {
                stk::mesh::Entity linked_entity_crs = link_data.get_linked_entity_crs(link_index, d);  // MOD_MARK
                stk::mesh::Entity linked_entity = link_data.get_linked_entity(link_index, d);          // MOD_MARK
  ...
  void gather_part_1_count(const stk::mesh::Selector &selector) {
    ...
    const stk::mesh::NgpMesh &ngp_mesh = stk::mesh::get_updated_ngp_mesh(link_data.bulk_data());
    const stk::mesh::EntityRank link_rank = link_data_ptr_->link_meta_data().link_rank();
    stk::NgpVector<unsigned> bucket_ids = ngp_mesh.get_bucket_ids(link_rank, link_subset_selector);
        ...
        team_policy, KOKKOS_LAMBDA(const TeamHandleType &team) {
          ...
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0u, num_links), [&](const int &i) {
            ...
            for (unsigned d = 0; d < dimensionality; ++d) {
              ...
              if (link_data.get_linked_entity(link_index, d) != stk::mesh::Entity()) {  // MOD_MARK
                stk::mesh::FastMeshIndex linked_entity_index =
                    link_data.get_linked_entity_index(link_index, d);                                        // MOD_MARK
                stk::mesh::EntityRank linked_entity_rank = link_data.get_linked_entity_rank(link_index, d);  // MOD_MARK
                auto &crs_bucket_conn =
  ...
  void finalize_crs_update(const stk::mesh::Selector &selector) {
    ...
    timer.reset();
    auto &link_needs_updated_field = link_data_ptr_->link_meta_data().link_crs_needs_updated_field();
    ...
    timer.reset();
    ::mundy::mesh::field_copy<entity_value_t>(link_data_ptr_->link_meta_data().linked_entities_field(),
                                              link_data_ptr_->link_meta_data().linked_entities_crs_field(),
  ...
  void check_coo_to_crs_conn(const stk::mesh::Selector &selector) {
    ...
    const NewNgpLinkDataT<NgpMemSpace> &link_data = *link_data_ptr_;
    const stk::mesh::NgpMesh &local_ngp_mesh = stk::mesh::get_updated_ngp_mesh(link_data.bulk_data());
    const NgpCRSPartitionView &partitions = get_or_create_crs_partitions(link_subset_selector);
    for (unsigned partition_id = 0; partition_id < partitions.extent(0); ++partition_id) {
      const NewNgpCRSPartition &partition = partitions(partition_id);
      ...
      stk::mesh::for_each_entity_run(
          local_ngp_mesh, link_rank, partition.selector(), KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex &linker_index) {
            ...
            for (unsigned d = 0; d < dimensionality; ++d) {
              stk::mesh::Entity linked_entity = link_data.get_linked_entity(linker_index, d);  // MOD_MARK
              if (linked_entity != stk::mesh::Entity()) {
                // Fetch the CRS connectivity of the linked entity
                stk::mesh::EntityRank linked_entity_rank =
                    link_data.get_linked_entity_rank(linker_index, d);  // MOD_MARK
                stk::mesh::FastMeshIndex linked_entity_index =
                    link_data.get_linked_entity_index(linker_index, d);  // MOD_MARK
  ...
  void check_crs_to_coo_conn(const stk::mesh::Selector &selector) {
    ...
    for (stk::topology::rank_t rank = stk::topology::NODE_RANK; rank < stk::topology::NUM_RANKS; ++rank) {
      stk::NgpVector<unsigned> bucket_ids =
          local_ngp_mesh.get_bucket_ids(rank, link_data.bulk_data().mesh_meta_data().universal_part());
            ...
            for (unsigned partition_id = 0; partition_id < num_partitions; ++partition_id) {
              ...
              Kokkos::parallel_for(Kokkos::TeamThreadRange(team, 0u, num_entities), [&](const int &i) {
                ...
                for (unsigned connected_link_ord = 0; connected_link_ord < connected_links.size();
                  ...
                  for (unsigned d = 0; d < dimensionality; ++d) {
                    stk::mesh::Entity linked_entity = link_data.get_linked_entity(connected_link_index, d);  // MOD_MARK
 ...
 private:
  ...
  NewNgpLinkDataT<NgpMemSpace> *link_data_ptr_;
...
class NewNgpLinkDataT {
 public:
  ...
  /// These aren't expensive operations and they're designed to be fast/GPU-compatible, but they aren't free.
  bool is_crs_connectivity_up_to_date(const stk::mesh::Selector &selector) {
    return ngp_crs_manager_.is_crs_connectivity_up_to_date(selector);
  }

  /// \brief Check if the CRS connectivity is up-to-date for all links.
  bool is_crs_connectivity_up_to_date() {
    return ngp_crs_manager_.is_crs_connectivity_up_to_date();
  }
  ...
  /// the CRS connectivity to reflect these changes.
  void update_crs_connectivity(const stk::mesh::Selector &selector) {
    ngp_crs_manager_.update_crs_connectivity(selector);
  }

  /// \brief Propagate changes made to the COO connectivity to the CRS connectivity.
  void update_crs_connectivity() {
    ngp_crs_manager_.update_crs_connectivity();
  }
