...
update_crs_connectivity should be done for all partitions all at once and should be managed by the NgpLinkData.
...
struct NewNgpCRSBucketConnT {
  using BucketConnectivityType = Kokkos::View<stk::mesh::Entity *, stk::ngp::MemSpace>;
...
struct NewNgpCRSPartitionT {
  ...
  using FastMeshIndex = stk::mesh::FastMeshIndex;
  using ConnectedEntities = stk::util::StridedArray<const stk::mesh::Entity>;
  //@}
