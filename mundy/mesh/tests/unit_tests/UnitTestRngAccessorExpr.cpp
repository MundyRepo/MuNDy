...
class UnitTestRngAccessorExprFixture : public ::testing::Test {
 public:
  ...
  stk::mesh::Field<Scalar>* create_field_on_parts(const std::string& field_name,
                                                  const stk::mesh::EntityRank& entity_rank, const int& num_components,
  ...
  void setup_hex_mesh(stk::mesh::BulkData::AutomaticAuraOption aura_option,
    ...
    seed_field_ptr_ =
        create_field_on_parts<size_t>("seed_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    counter_field_ptr_ =
        create_field_on_parts<size_t>("counter_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);

    lower_bound_d_field_ptr_ =
        create_field_on_parts<double>("lower_bound_d_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    upper_bound_d_field_ptr_ =
        create_field_on_parts<double>("upper_bound_d_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    lower_bound_f_field_ptr_ =
        create_field_on_parts<float>("lower_bound_f_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    upper_bound_f_field_ptr_ =
        create_field_on_parts<float>("upper_bound_f_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    lower_bound_i_field_ptr_ =
        create_field_on_parts<int>("lower_bound_i_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    upper_bound_i_field_ptr_ =
        create_field_on_parts<int>("upper_bound_i_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);

    double_field_ptr_ =
        create_field_on_parts<double>("double_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    float_field_ptr_ =
        create_field_on_parts<float>("float_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);
    int_field_ptr_ = create_field_on_parts<int>("int_field", stk::topology::NODE_RANK, scalars_per_entity, all_blocks);

    // Extra fields to add heterogeneity
    field1_ptr_ =
        create_field_on_parts<double>("field1", stk::topology::NODE_RANK, scalars_per_entity, {block1_part_ptr_});
    field2_ptr_ =
        create_field_on_parts<double>("field2", stk::topology::NODE_RANK, scalars_per_entity, {block2_part_ptr_});
    field3_ptr_ =
 ...
 protected:
  ...
  stk::mesh::Field<double>* lower_bound_d_field_ptr_;
  stk::mesh::Field<double>* upper_bound_d_field_ptr_;
  stk::mesh::Field<float>* lower_bound_f_field_ptr_;
  stk::mesh::Field<float>* upper_bound_f_field_ptr_;
  stk::mesh::Field<int>* lower_bound_i_field_ptr_;
  stk::mesh::Field<int>* upper_bound_i_field_ptr_;
...
void randomize_test(stk::mesh::BulkData& bulk_data,           //
  ...
  if constexpr (use_seed_expr && use_counter_expr) {
    ...
    static_assert(use_seed_expr != false && use_counter_expr != false,
  ...
  ::mundy::mesh::for_each_entity_run(
      ...
      [&double_accessor, &float_accessor, &int_accessor, &seed, &counter, fixed_seed, fixed_counter](
        ...
        openrand::Philox rng_d = utils::make_philox(local_seed, local_counter);
        ...
        openrand::Philox rng_f = utils::make_philox(local_seed, local_counter);
        ...
        openrand::Philox rng_i = utils::make_philox(local_seed, local_counter);
...
TEST_F(UnitTestRngAccessorExprFixture, field_rand) {
  ...
  randomize_test<true, true>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_, *float_field_ptr_,
                             *int_field_ptr_, b1_not_b2);

  randomize_test<true, false>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_, *float_field_ptr_,
                              *int_field_ptr_, b1_not_b2);

  randomize_test<false, true>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_, *float_field_ptr_,
                              *int_field_ptr_, b1_not_b2);
}

template <bool use_lower_bound_expr, bool use_upper_bound_expr>
void randomize_uniform_test(stk::mesh::BulkData& bulk_data,             //
                            stk::mesh::Field<size_t>& seed_field,       //
                            stk::mesh::Field<size_t>& counter_field,    //
                            stk::mesh::Field<double>& double_field,     //
                            stk::mesh::Field<double>& double_lb_field,  //
                            stk::mesh::Field<double>& double_ub_field,  //
                            stk::mesh::Field<float>& float_field,       //
                            stk::mesh::Field<float>& float_lb_field,    //
                            stk::mesh::Field<float>& float_ub_field,    //
                            stk::mesh::Field<int>& int_field,           //
                            stk::mesh::Field<int>& int_lb_field,        //
  ...
  double double_fixed_ub = 23.4;
  float float_fixed_lb = -5.5f;
  float float_fixed_ub = 12.3f;
  int int_fixed_lb = -10;
  int int_fixed_ub = 20;
  auto es = make_entity_expr(bulk_data, selector, stk::topology::NODE_RANK);
  if constexpr (use_lower_bound_expr && use_upper_bound_expr) {
    auto our_rng = rng(seed(es), counter(es));
    double_accessor(es) = our_rng.template uniform<double>(double_lb_accessor(es), double_ub_accessor(es));
    float_accessor(es) = our_rng.template uniform<float>(float_lb_accessor(es), float_ub_accessor(es));
    int_accessor(es) = our_rng.template uniform<int>(int_lb_accessor(es), int_ub_accessor(es));
  } else if constexpr (use_lower_bound_expr && !use_upper_bound_expr) {
    auto our_rng = rng(seed(es), counter(es));
    double_accessor(es) = our_rng.template uniform<double>(double_lb_accessor(es), double_fixed_ub);
    float_accessor(es) = our_rng.template uniform<float>(float_lb_accessor(es), float_fixed_ub);
    int_accessor(es) = our_rng.template uniform<int>(int_lb_accessor(es), int_fixed_ub);
  } else if constexpr (!use_lower_bound_expr && use_upper_bound_expr) {
    auto our_rng = rng(seed(es), counter(es));
    double_accessor(es) = our_rng.template uniform<double>(double_fixed_lb, double_ub_accessor(es));
    float_accessor(es) = our_rng.template uniform<float>(float_fixed_lb, float_ub_accessor(es));
    int_accessor(es) = our_rng.template uniform<int>(int_fixed_lb, int_ub_accessor(es));
  } else {
    static_assert(use_lower_bound_expr != false && use_upper_bound_expr != false,
                  "Test will not compile unless at least one of use_lower_bound_expr or use_upper_bound_expr is true.");
  }
  ...
  ::mundy::mesh::for_each_entity_run(
      ...
      [&double_accessor, &float_accessor, &int_accessor, &double_lb_accessor, &double_ub_accessor, &float_lb_accessor,
       ...
       int_fixed_ub](const stk::mesh::BulkData& /*bulk_data*/, const stk::mesh::Entity& e) {
        ...
        int expected_value_i = rng_i.uniform<int>(lb_i, ub_i);
        EXPECT_EQ(actual_value_i, expected_value_i);
      });
}

TEST_F(UnitTestRngAccessorExprFixture, field_uniform) {
  if (stk::parallel_machine_size(communicator_) > 2) {
    GTEST_SKIP() << "This test is only designed to run with 1 or 2 MPI ranks.";
  }
  ...
  stk::mesh::Selector b1_not_b2 = block1_selector_ - block2_selector_;
  randomize_uniform_test<true, true>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_,
                                     *lower_bound_d_field_ptr_, *upper_bound_d_field_ptr_, *float_field_ptr_,
                                     *lower_bound_f_field_ptr_, *upper_bound_f_field_ptr_, *int_field_ptr_,
                                     *lower_bound_i_field_ptr_, *upper_bound_i_field_ptr_, b1_not_b2);

  randomize_uniform_test<true, false>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_,
                                      *lower_bound_d_field_ptr_, *upper_bound_d_field_ptr_, *float_field_ptr_,
                                      *lower_bound_f_field_ptr_, *upper_bound_f_field_ptr_, *int_field_ptr_,
                                      *lower_bound_i_field_ptr_, *upper_bound_i_field_ptr_, b1_not_b2);

  randomize_uniform_test<false, true>(get_bulk(), *seed_field_ptr_, *counter_field_ptr_, *double_field_ptr_,
                                      *lower_bound_d_field_ptr_, *upper_bound_d_field_ptr_, *float_field_ptr_,
                                      *lower_bound_f_field_ptr_, *upper_bound_f_field_ptr_, *int_field_ptr_,
                                      *lower_bound_i_field_ptr_, *upper_bound_i_field_ptr_, b1_not_b2);
}
