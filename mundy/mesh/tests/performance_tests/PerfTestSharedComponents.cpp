...
static matrix3_t make_spd_matrix3(value_type b) {
  return matrix3_t(1.25 + b, 0.08 + 0.05 * b, -0.04 + 0.01 * b, 0.08 + 0.05 * b, 1.75 + 0.50 * b, 0.06 - 0.02 * b,
...
template <typename DtT, typename AmbientT, typename DragT, typename TargetT, typename ForceT, typename TorqueT,
          typename OrientT, typename MobilityT, typename VelT, typename StressT, typename OrientOutT, typename EnergyT>
KOKKOS_INLINE_FUNCTION void rigid_body_step(const DtT& dt, const AmbientT& ambient, const DragT& drag,
                                            ...
                                            const OrientT& orient, const MobilityT& mobility, VelT vel, StressT stress,
                                            OrientOutT orient_out, EnergyT energy) {
  ...
  energy = mundy::dot(lab_vel, lab_vel) + 0.1 * mundy::dot(body_spin, body_spin) + mundy::dot(trial_orient, target);
}
...
static void validate_equal(const std::string& label, RunnerA&& a, RunnerB&& b, const RigidBodyFixture& f,
  ...
  if (std::abs(sum_a - sum_b) > 1.0e-11 * scale)
    throw std::runtime_error("validate_equal failed for \"" + label + "\": " + std::to_string(sum_a) + " vs " +
                             std::to_string(sum_b));
}
...
static std::string fmt_time(double ns) {
  ...
  if (ns < 1e3)
    ...
    std::snprintf(buf, sizeof(buf), "%7.2f us", ns * 1e-3);
  else
    std::snprintf(buf, sizeof(buf), "%7.2f ms", ns * 1e-6);
  return buf;
}
...
static void print_simple_table(const std::string& title, const std::vector<VariantSummary>& rows) {
  ...
  std::cout << "\n[" << title << "]  (median ns/entity)\n"
            << "  " << std::left << std::setw(kLabelW) << "variant" << std::right << std::setw(kValW) << "time" << "\n"
            << "  " << std::string(kLabelW + kValW, '-') << "\n";
  for (const auto& r : rows)
    std::cout << "  " << std::left << std::setw(kLabelW) << r.label << std::right << std::setw(kValW)
              << fmt_time(r.ns_per_entity) << "\n";
}
...
static void bench_drag_host(const BenchOptions& opts) {
  ...
  b.run("field", [&] {
    run_field();
    ankerl::nanobench::doNotOptimizeAway(*f.velocity);
  });
  b.run("shared", [&] {
    run_shared();
    ankerl::nanobench::doNotOptimizeAway(*f.velocity);
  ...
  if (opts.simple)
    print_simple_table("Shared components / Host drag", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
void eval_drag_velocity_step(NgpMesh ngp_mesh, stk::mesh::Selector& selector, ForceComp ngp_force_comp,
  ...
  mundy::mesh::for_each_entity_run(
      ngp_mesh, stk::topology::NODE_RANK, selector, KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex& idx) {
...
static void bench_drag_ngp(const BenchOptions& opts) {
  ...
  auto run = [&](auto drag) {
    // Can't have a KOKKOS_LAMBDA inside a generic lambda (CUDA restriction), so we need a separate function for the NGP
    // drag step.
    eval_drag_velocity_step(ngp_mesh, f.selector, ngp_force_comp, ngp_vel_comp, drag);
  };
  auto run_field = [&] {
    ngp_force_comp.sync_to_device();
    ngp_vel_comp.sync_to_device();
    ngp_drag_fc.sync_to_device();
    run(ngp_drag_fc);
    ngp_vel_comp.modify_on_device();
    Kokkos::fence();
  };
  auto run_shared = [&] {
    ngp_force_comp.sync_to_device();
    ngp_vel_comp.sync_to_device();
    ngp_drag_sc.sync_to_device();
    run(ngp_drag_sc);
    ngp_vel_comp.modify_on_device();
    Kokkos::fence();
  };
  ...
  b.run("field", [&] {
    run_field();
    ankerl::nanobench::doNotOptimizeAway(ngp_vel_comp);
  });
  b.run("shared", [&] {
    run_shared();
    ankerl::nanobench::doNotOptimizeAway(ngp_vel_comp);
  ...
  if (opts.simple)
    print_simple_table("Shared components / NGP drag", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
static void bench_mobility_host(const BenchOptions& opts) {
  ...
  auto run = [&](auto ambient, auto mobility) {
    mundy::mesh::for_each_entity_run(
        *f.bulk, stk::topology::NODE_RANK, f.selector, [=](const stk::mesh::BulkData&, stk::mesh::Entity e) {
  ...
  if (opts.simple)
    print_simple_table("Shared components / Host mobility", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
void eval_rigid_body_mobility_step(NgpMesh ngp_mesh, stk::mesh::Selector& selector, AmbientT ngp_ambient,
  ...
  mundy::mesh::for_each_entity_run(
      ngp_mesh, stk::topology::NODE_RANK, selector, KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex& idx) {
        rigid_body_mobility_step(ngp_ambient(idx), ngp_mobility(idx), ngp_force_comp(idx), ngp_orient_comp(idx),
                                 ngp_vel_comp(idx));
      });
}
...
static void bench_mobility_ngp(const BenchOptions& opts) {
  ...
  auto run = [&](auto ambient, auto mobility) {
    eval_rigid_body_mobility_step(ngp_mesh, f.selector, ambient, mobility, ngp_force_comp, ngp_orient_comp,
                                  ngp_vel_comp);
  };
  auto run_field = [&] {
    ngp_force_comp.sync_to_device();
    ngp_orient_comp.sync_to_device();
    ngp_vel_comp.sync_to_device();
    ngp_ambient_fc.sync_to_device();
    ngp_mobility_fc.sync_to_device();
  ...
  auto run_shared = [&] {
    ...
    ngp_orient_comp.sync_to_device();
    ngp_vel_comp.sync_to_device();
    ngp_ambient_sc.sync_to_device();
    ngp_mobility_sc.sync_to_device();
  ...
  if (opts.simple)
    print_simple_table("Shared components / NGP mobility", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
static void bench_complex_host(const BenchOptions& opts) {
  ...
  FC_v ambient_fc(*f.ambient);
  FC_m drag_fc(*f.drag);
  FC_q target_fc(*f.target_orientation);
  SC_s dt_sc(f.shared_dt);
  SC_v ambient_sc(f.shared_ambient);
  SC_m drag_sc(f.shared_drag);
  SC_q target_sc(f.shared_target_orientation);

  auto run = [&](auto dt, auto ambient, auto drag, auto target) {
    mundy::mesh::for_each_entity_run(
        *f.bulk, stk::topology::NODE_RANK, f.selector, [=](const stk::mesh::BulkData&, stk::mesh::Entity e) {
          rigid_body_step(dt(e), ambient(e), drag(e), target(e), force_comp(e), torque_comp(e), orient_comp(e),
                          mobility_comp(e), vel_comp(e), stress_comp(e), orient_out_comp(e), energy_comp(e));
        });
  };
  ...
  b.run("field", [&] {
    run_field();
    ankerl::nanobench::doNotOptimizeAway(*f.energy);
  });
  b.run("shared", [&] {
    run_shared();
    ankerl::nanobench::doNotOptimizeAway(*f.energy);
  ...
  if (opts.simple)
    print_simple_table("Shared components / Host complex", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
void eval_rigid_body_step(NgpMesh ngp_mesh, stk::mesh::Selector& selector, DtT dt, AmbientT ambient, DragT drag,
  ...
  mundy::mesh::for_each_entity_run(
      ngp_mesh, stk::topology::NODE_RANK, selector, KOKKOS_LAMBDA(const stk::mesh::FastMeshIndex& idx) {
        rigid_body_step(dt(idx), ambient(idx), drag(idx), target(idx), ngp_force_comp(idx), ngp_torque_comp(idx),
                        ngp_orient_comp(idx), ngp_mobility_comp(idx), ngp_vel_comp(idx), ngp_stress_comp(idx),
...
static void bench_complex_ngp(const BenchOptions& opts) {
  ...
  auto sync_per_entity_comps = [&] {
    ngp_force_comp.sync_to_device();
    ngp_torque_comp.sync_to_device();
    ngp_orient_comp.sync_to_device();
    ngp_mobility_comp.sync_to_device();
    ngp_vel_comp.sync_to_device();
    ngp_stress_comp.sync_to_device();
    ngp_orient_out_comp.sync_to_device();
    ngp_energy_comp.sync_to_device();
  };
  auto mark_outputs_modified = [&] {
    ngp_vel_comp.modify_on_device();
    ngp_stress_comp.modify_on_device();
    ngp_orient_out_comp.modify_on_device();
    ngp_energy_comp.modify_on_device();
    Kokkos::fence();
  };

  auto run = [&](auto dt, auto ambient, auto drag, auto target) {
    eval_rigid_body_step(ngp_mesh, f.selector, dt, ambient, drag, target, ngp_force_comp, ngp_torque_comp,
                         ngp_orient_comp, ngp_mobility_comp, ngp_vel_comp, ngp_stress_comp, ngp_orient_out_comp,
                         ngp_energy_comp);
  };
  auto run_field = [&] {
    sync_per_entity_comps();
    ngp_dt_fc.sync_to_device();
    ngp_ambient_fc.sync_to_device();
    ngp_drag_fc.sync_to_device();
    ngp_target_fc.sync_to_device();
    run(ngp_dt_fc, ngp_ambient_fc, ngp_drag_fc, ngp_target_fc);
    mark_outputs_modified();
  };
  auto run_shared = [&] {
    sync_per_entity_comps();
    ngp_dt_sc.sync_to_device();
    ngp_ambient_sc.sync_to_device();
    ngp_drag_sc.sync_to_device();
    ngp_target_sc.sync_to_device();
  ...
  b.run("field", [&] {
    run_field();
    ankerl::nanobench::doNotOptimizeAway(ngp_energy_comp);
  });
  b.run("shared", [&] {
    run_shared();
    ankerl::nanobench::doNotOptimizeAway(ngp_energy_comp);
  ...
  if (opts.simple)
    print_simple_table("Shared components / NGP complex", collect_summaries(b, opts.num_entities));
  else
    std::cout << out.str();
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--case" && i + 1 < argc) {
      ...
      if (v == "drag")
        opts.benchmark_case = BenchmarkCase::Drag;
      else if (v == "mobility")
        opts.benchmark_case = BenchmarkCase::Mobility;
      else if (v == "complex")
        opts.benchmark_case = BenchmarkCase::Complex;
      else if (v == "all")
        opts.benchmark_case = BenchmarkCase::All;
      else {
        std::cerr << "Unknown --case value: " << v << "\n";
        return 1;
      }
      continue;
    }
    if (arg == "--space" && i + 1 < argc) {
      const std::string v(argv[++i]);
      if (v == "host")
        opts.execution_space = ExecutionSpace::Host;
      else if (v == "ngp")
        opts.execution_space = ExecutionSpace::Ngp;
      else if (v == "all")
        opts.execution_space = ExecutionSpace::All;
      else {
        std::cerr << "Unknown --space value: " << v << "\n";
        return 1;
      }
      continue;
    }
