...
class PeriodicSpaceMetric {
 public:
  ...
  explicit constexpr PeriodicSpaceMetric(const mundy::Matrix3<Scalar>& h) : h_(h), h_inv_(mundy::inverse(h)) {
  }

  /// \brief Set the unit cell matrix
  KOKKOS_INLINE_FUNCTION
  void constexpr set_unit_cell_matrix(const mundy::Matrix3<Scalar>& h) {
    h_ = h;
    h_inv_ = mundy::inverse(h_);
  }
 ...
 private:
  mundy::Matrix3<Scalar> h_;      ///< Unit cell matrix
  mundy::Matrix3<Scalar> h_inv_;  ///< Inverse of the unit cell matrix
};  // PeriodicSpaceMetric
...
class PeriodicScaledSpaceMetric {
 public:
  ...
  /// \brief Constructor with unit cell matrix
  KOKKOS_INLINE_FUNCTION
  explicit constexpr PeriodicScaledSpaceMetric(const mundy::Vector3<Scalar>& cell_size)
      : scale_(cell_size), scale_inv_(Scalar(1.0) / scale_[0], Scalar(1.0) / scale_[1], Scalar(1.0) / scale_[2]) {
  }

  /// \brief Set the cell size
  KOKKOS_INLINE_FUNCTION
  void set_cell_size(const mundy::Vector3<Scalar>& cell_size) {
    scale_ = cell_size;
  ...
  constexpr Point<Scalar> operator()(const Point<Scalar>& point1, const Point<Scalar>& point2) const {
    ...
    // Map the fractional coordinates back to real space
    return mundy::elementwise_mul(scale_, ds_frac);
  }

 private:
  mundy::Vector3<Scalar> scale_;      ///< Unit cell scaling factors
  mundy::Vector3<Scalar> scale_inv_;  ///< Inverse of the scaling factors
...
KOKKOS_INLINE_FUNCTION constexpr PeriodicSpaceMetric<Scalar> periodic_metric_from_unit_cell(
  ...
  auto h = mundy::Matrix3<Scalar>::identity();
  h(0, 0) = cell_size[0];
  h(1, 1) = cell_size[1];
  h(2, 2) = cell_size[2];
