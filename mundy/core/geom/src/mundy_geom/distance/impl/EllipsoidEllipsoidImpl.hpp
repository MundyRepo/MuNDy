...
class EllipsoidEllipsoidObjective {
 public:
  ...
  KOKKOS_FUNCTION
  EllipsoidEllipsoidObjective(const EllipsoidType1& e0, const EllipsoidType2& e1, mundy::Vector3<Scalar>& sn0,
                              mundy::Vector3<Scalar>& sn1, Point<Scalar>& fp0, Point<Scalar>& fp1)
...
class EllipsoidEllipsoidObjectiveFDF {
 public:
  ...
  KOKKOS_FUNCTION
  EllipsoidEllipsoidObjectiveFDF(const EllipsoidType1& e0, const EllipsoidType2& e1, mundy::Vector3<Scalar>& sn0,
                                 mundy::Vector3<Scalar>& sn1, Point<Scalar>& fp0, Point<Scalar>& fp1)
      : e0_(e0), e1_(e1), sn0_(sn0), sn1_(sn1), fp0_(fp0), fp1_(fp1) {
  }

  KOKKOS_FUNCTION Scalar operator()(const mundy::Vector<Scalar, 2>& tp, mundy::Vector<Scalar, 2>& g) const {
    const Scalar sth = sin(tp[0]), cth = cos(tp[0]);
    const Scalar sph = sin(tp[1]), cph = cos(tp[1]);
    ...
    const mundy::Vector3<Scalar> grad_n = jac(e0_, n, fp0_, unit_d) + jac(e1_, -n, fp1_, unit_d);
 ...
 private:
  ...
  KOKKOS_FUNCTION static mundy::Vector3<Scalar> jac(const E& e, const mundy::Vector3<Scalar>& n_lab,
                                                    const Point<Scalar>& p_lab, const mundy::Vector3<Scalar>& unit_d) {
