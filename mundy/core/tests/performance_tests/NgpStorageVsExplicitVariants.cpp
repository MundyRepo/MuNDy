...
/// \brief Performance test: explicit owned/pointer/reference workspaces vs mundy::core::storage variants using
  ...
  for (size_t round = 0; round < rounds; ++round) {
    Kokkos::parallel_for(
        "run_storage_pointer", Kokkos::RangePolicy<>(0, n), KOKKOS_LAMBDA(const size_t i) {
          WorkspaceStorage workspace{mundy::core::store(x_data + i), mundy::core::store(y_data + i),
                                     mundy::core::store(z_data + i)};
          workspace.step(alpha, beta, i, round);
        });
  }
  Kokkos::fence();
