...
template <class T>
concept can_ref = requires(T&& t) { ::mundy::core::ref(static_cast<T&&>(t)); };
...
KOKKOS_FUNCTION constexpr bool constexpr_get_and_conversion_work() {
  int value = 2;
  ::mundy::core::reference_wrapper<int> wrapper(value);
  int& as_ref = wrapper;
  as_ref += 5;
...
KOKKOS_FUNCTION constexpr bool constexpr_ref_and_cref_work() {
  ...
  auto const_wrapped = ::mundy::core::cref(const_value);
...
KOKKOS_FUNCTION constexpr ::mundy::core::reference_wrapper<int> make_wrapper_from_scope(int& value) {
  return ::mundy::core::ref(value);
}

KOKKOS_FUNCTION constexpr ::mundy::core::reference_wrapper<const int> make_const_wrapper_from_scope(const int& value) {
  return ::mundy::core::cref(value);
}
...
TEST(ReferenceWrapperTest, RefAndCrefHelpersWrapLvalues) {
  int value = 5;
  auto mutable_wrapper = ::mundy::core::ref(value);
  static_assert(std::is_same_v<decltype(mutable_wrapper), ::mundy::core::reference_wrapper<int>>,
                "ref(int&) should produce reference_wrapper<int>");

  mutable_wrapper.get() = 9;
  EXPECT_EQ(value, 9);

  auto const_wrapper = ::mundy::core::cref(value);
  static_assert(std::is_same_v<decltype(const_wrapper), ::mundy::core::reference_wrapper<const int>>,
...
TEST(ReferenceWrapperTest, RefAndCrefFromExistingWrappersPreserveReference) {
  int value = 4;
  auto wrapper = ::mundy::core::ref(value);

  auto wrapper_again = ::mundy::core::ref(wrapper);
  wrapper_again.get() = 12;
  EXPECT_EQ(value, 12);

  auto const_from_wrapper = ::mundy::core::cref(wrapper);
  EXPECT_EQ(const_from_wrapper.get(), 12);

  auto const_from_const_wrapper = ::mundy::core::cref(const_from_wrapper);
  EXPECT_EQ(const_from_const_wrapper.get(), 12);
}

TEST(ReferenceWrapperTest, WrapperCopySemanticsStillReferenceSameObject) {
  int value = 21;
  ::mundy::core::reference_wrapper<int> first(value);
  ::mundy::core::reference_wrapper<int> second(first);

  second.get() = 42;
  EXPECT_EQ(value, 42);

  int other = -1;
  ::mundy::core::reference_wrapper<int> third(other);
  third = first;
  third.get() = 77;
  EXPECT_EQ(value, 77);
  EXPECT_EQ(other, -1);
...
TEST(ReferenceWrapperTest, CallableForwardingWorksForMutableCallable) {
  ...
  auto wrapped = ::mundy::core::ref(accumulator);
...
TEST(ReferenceWrapperTest, TypeTraitsAndDeductionGuide) {
  static_assert(::mundy::core::is_reference_wrapper_v<::mundy::core::reference_wrapper<int>>,
                "reference_wrapper<T> should be detected as a wrapper");
  static_assert(!::mundy::core::is_reference_wrapper_v<int>, "non-wrapper types should not be detected as wrappers");

  int value = 8;
  ::mundy::core::reference_wrapper deduced(value);
  static_assert(std::is_same_v<decltype(deduced), ::mundy::core::reference_wrapper<int>>,
...
TEST(ReferenceWrapperTest, ConversionAndConstructibilityContracts) {
  static_assert(std::is_convertible_v<::mundy::core::reference_wrapper<int>, int&>,
                "reference_wrapper<int> should be implicitly convertible to int&");
  static_assert(std::is_convertible_v<::mundy::core::reference_wrapper<const int>, const int&>,
                "reference_wrapper<const int> should be implicitly convertible to const int&");
  static_assert(!std::is_convertible_v<::mundy::core::reference_wrapper<const int>, int&>,
...
TEST(ReferenceWrapperTest, ConstWrapperAllowsReadOnlyAccess) {
  int value = 13;
  const ::mundy::core::reference_wrapper<int> wrapper(value);

  EXPECT_EQ(wrapper.get(), 13);
  wrapper.get() = 14;
  EXPECT_EQ(value, 14);

  const auto const_wrapper = ::mundy::core::cref(value);
...
TEST(ReferenceWrapperTest, CallableForwardingSupportsReferenceReturnAndConstCallable) {
  ...
  const auto const_wrapped = ::mundy::core::cref(callable);
