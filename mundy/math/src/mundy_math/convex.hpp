...
concept HasWorkspaceType = requires { typename std::remove_reference_t<LinearOp>::Workspace; };

template <class LinearOp, class Vector, bool HasWorkspace = HasWorkspaceType<LinearOp>>
struct workspace_for_impl;

template <class LinearOp, class Vector>
struct workspace_for_impl<LinearOp, Vector, true> {
  using type = typename std::remove_reference_t<LinearOp>::Workspace;
};

template <class LinearOp, class Vector>
struct workspace_for_impl<LinearOp, Vector, false> {
  using type = decltype(make_workspace(std::declval<const LinearOp&>(), std::declval<const Vector&>()));
};

template <class LinearOp, class Vector>
using workspace_for_t = typename workspace_for_impl<LinearOp, Vector>::type;
...
template <typename Backend, typename LinearOp, typename QVector, typename ConvexSpace,
          typename Workspace = impl::workspace_for_t<LinearOp, QVector>>
class CQPPProblem {
 public:
  using backend_t = Backend;
...
class LCPProblem {
 public:
  ...
  LCPProblem(Backend, const linear_op_t& A, const vector_t& q) : A_(A), q_(q), workspace_(impl::make_workspace(A, q)) {
  }
...
class PGDStrategy {
 public:
  ...
  KOKKOS_INLINE_FUNCTION void initialize(const Problem& prob, State& state) const {
    ...
    // x_tmp = x
    decltype(backend)::deep_copy(state.x_tmp(), state.x());

    // grad_tmp = A x_tmp + q
    impl::workspace_invalidate(workspace);
    decltype(backend)::apply(prob.A(), state.x_tmp(), state.grad_tmp(), workspace);
    decltype(backend)::axpby(one, prob.q(), one, state.grad_tmp());
    ...
    if (state.converged()) {
      ...
      decltype(backend)::deep_copy(state.grad(), state.grad_tmp());
  ...
  KOKKOS_INLINE_FUNCTION bool iterate(const Problem& prob, State& state) const {
    ...
    decltype(backend)::wrapped_axpbyz(one, state.x_tmp(), -state.step_size(), state.grad_tmp(), state.x(),
                                      prob.space());

    // grad = A x + q
    impl::workspace_invalidate(workspace);
    decltype(backend)::apply(prob.A(), state.x(), state.grad(), workspace);
    decltype(backend)::axpby(one, prob.q(), one, state.grad());
    ...
    decltype(backend)::deep_copy(state.x_tmp(), state.x());
    decltype(backend)::deep_copy(state.grad_tmp(), state.grad());
    ++state.iter();
    return false;
  }
