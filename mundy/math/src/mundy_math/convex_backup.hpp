...
class MCQPPProblem {
 public:
  ...
  KOKKOS_INLINE_FUNCTION
  MCQPPProblem(Backend, const linear_op_t& A, const vector_t& q, const space_t& space) : A_(A), q_(q), space_(space) {
  }
...
concept IterativeSolverStrategy = requires(const Strategy& s, const Problem& prob, typename Strategy::state_t& state) {
...
//
template <class Backend, class StepPolicy, class ResidualPolicy>
KOKKOS_INLINE_FUNCTION auto make_pgd_solution_strategy(
...
template <class Problem, class Strategy>
  requires convex::IterativeSolverStrategy<Strategy, Problem>
...
///
/// In the less-general problem above: L = B A^{-1} B^T and f_b = B A^{-1} b. In this form, the reduced CQPP is:
///   x^* = argmin_{x in Omega_x} 0.5 x^T H x + g^T x
///   H := D^T M D - D^T M L M D
///   g := q - D^T M f_b
...
template <class Problem, class Strategy>
  requires convex::IterativeSolverStrategy<Strategy, Problem>
KOKKOS_INLINE_FUNCTION auto solve_mcqpp(const Problem& prob, const Strategy& strat, typename Strategy::state_t& state)
    -> typename Strategy::result_t {
  strat.initialize(prob, state);
  while (!strat.done(state)) {
    if (strat.iterate(prob, state)) break;
