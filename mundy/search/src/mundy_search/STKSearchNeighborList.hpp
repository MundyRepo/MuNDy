...
/// source entities.  Because the source input is a component, the build *does* communicate the source
/// component's geometry field over the ghosting group `"MUNDY_STK_SEARCH_NL_GHOSTING"`, so ghosted sources
/// carry valid geometry.  The group persists on `bulk_data` after the call returns; callers communicate any
/// *additional* fields they need (e.g. per-source application data) over this same group before computing
/// interactions.  On single-rank runs no ghosting occurs and the group is not created.
///
/// \par `list.source_entities()` superset
...
template <typename Builder>
  requires PeriodicAABBSearchInputType<typename Builder::target_input_type> &&
           PeriodicAABBSearchInputType<typename Builder::source_input_type>
PeriodicSTKSearchNeighborList<MemorySpace, ImageShiftScalar>
NeighborListBuildTraits<PeriodicSTKSearchNeighborList<MemorySpace, ImageShiftScalar>>::build(
    const Builder& builder, const stk::mesh::BulkData& bulk_data, const args_type& /*args*/) {
  using list_type = PeriodicSTKSearchNeighborList<MemorySpace, ImageShiftScalar>;
  using exec_space = typename Builder::execution_space;
  using size_type = typename list_type::size_type;
