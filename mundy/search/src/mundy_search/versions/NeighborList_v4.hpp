...
struct ExcludeSelfInteraction {
 ...
 private:
  ...
  template <typename Candidate>
  KOKKOS_INLINE_FUNCTION
  static bool relative_shift_is_zero(const Candidate& /*candidate*/) {
    return true;
  }
  ...
  KOKKOS_INLINE_FUNCTION
  static bool relative_shift_is_zero(const PeriodicNeighborSearchCandidate<ImageShiftType, SizeType>& candidate) {
...
class ExcludeSymmetricDuplicates {
 public:
  ...
  bool operator()(const Candidate& candidate) const {
    ...
    MUNDY_THROW_ASSERT(trg_bucket_id < num_buckets_, std::runtime_error, "ExcludeSymmetricDuplicates: target bucket ID out of range.");
    MUNDY_THROW_ASSERT(src_bucket_id < num_buckets_, std::runtime_error, "ExcludeSymmetricDuplicates: source bucket ID out of range.");
 ...
 public:
  ...
  class EntityIterator {
   public:
    ...
    KOKKOS_INLINE_FUNCTION
    stk::mesh::Entity operator*() const { return list_.get_neighbor(target_index_, neighbor_ordinal_); }

    KOKKOS_INLINE_FUNCTION
    EntityIterator& operator++() { ++neighbor_ordinal_; return *this; }

    KOKKOS_INLINE_FUNCTION
    EntityIterator operator++(int) { auto tmp = *this; ++neighbor_ordinal_; return tmp; }

    KOKKOS_INLINE_FUNCTION
    bool operator==(const EntityIterator& other) const { return neighbor_ordinal_ == other.neighbor_ordinal_; }
  ...
  KOKKOS_INLINE_FUNCTION
  EntityIterator begin() const { return EntityIterator(list_, target_index_, 0); }

  /// \brief Return a past-the-end iterator over neighbor entities.
  KOKKOS_INLINE_FUNCTION
  EntityIterator end() const { return EntityIterator(list_, target_index_, size()); }
  //@}
...
void for_each_neighbor_pair(const ExecutionSpace& exec_space, const ListType& list, const Functor& functor) {
  ...
  Kokkos::parallel_for("mundy::mesh::for_each_neighbor_pair",
...
void for_each_target_with_neighbors(const ExecutionSpace& exec_space, const ListType& list, const Functor& functor) {
  ...
  Kokkos::parallel_for("mundy::mesh::for_each_target_with_neighbors",
                       range_policy_t(exec_space, 0, list.num_targets()), deploy_functor);
}
...
class NeighborListBuilder {
 public:
  ...
  /// \brief Construct a builder with the default empty exclusion chain.
  ...
  NeighborListBuilder(const execution_space& exec_space, const target_search_boxes_type& targets,
      ...
      : exec_space_(exec_space), targets_(targets), sources_(sources), exclusions_() {
  }
  ...
  /// \param exclusions [in] Statically typed exclusion chain.
  NeighborListBuilder(const execution_space& exec_space, const target_search_boxes_type& targets,
                      const source_search_boxes_type& sources, const exclusions_type& exclusions)
      : exec_space_(exec_space), targets_(targets), sources_(sources), exclusions_(exclusions) {
  }
  //@}
  ...
  auto exclude(const Exclusion& exclusion) const {
    ...
    return NeighborListBuilder<neighbor_list_type, execution_space, target_search_boxes_type, source_search_boxes_type,
                               new_exclusions_type>(exec_space_, targets_, sources_, new_exclusions);
  }

  /// \brief Return a new builder type with symmetric duplicate pair suppression appended.
  ///
  /// Uses the selectors stored in the target and source search boxes to determine the selector-overlap
  /// relationship and constructs the appropriate builder-prepared exclusion. All three cases (identical,
  /// disjoint, overlapping selectors) are handled correctly by impl::ExcludeSymmetricDuplicates.
  ...
  auto exclude_symmetric_duplicates(const stk::mesh::BulkData& bulk_data) const {
    return exclude(impl::ExcludeSymmetricDuplicates(bulk_data, targets_.selector(), sources_.selector()));
  }
  //@}
...
make_neighbor_list_builder(
  ...
  return NeighborListBuilder<NeighborListType, ExecutionSpace, TargetSearchBoxes, SourceSearchBoxes, NoExclusions>(
      exec_space, targets, sources, NoExclusions{});
}
...
ArborX1dNeighborList<MemorySpace> make_arborx_1d_neighbor_list(
    ...
    const impl::ArborXSearchBoxesT<MemorySpace>& sources, int buffer_size = 0);
