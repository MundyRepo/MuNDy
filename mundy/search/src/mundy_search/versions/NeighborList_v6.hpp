...
/// \brief Specifies a build-time exclusion object that can prepare itself for selected target/source chunks.
///
/// Excluders are stored in the builder's statically typed exclusion chain. `setup(...)` is called during neighbor-list
/// construction before the chain is copied into backend callbacks, giving each excluder a chance to cache selector- and
/// mesh-dependent state while keeping pair filtering concrete and Kokkos-callable.
template <typename T>
concept ExcluderType =
...
class NoExclusions {
 public:
  ...
  /// \brief Prepare the empty exclusion chain.
  ...
  /// \tparam NewExclusion Excluder type to append.
  /// \param exclusion [in] Exclusion predicate to append to the chain.
  template <ExcluderType NewExclusion>
...
template <typename PriorExclusions, typename Exclusion>
class ExclusionChain {
  static_assert(ExcluderType<PriorExclusions>, "ExclusionChain requires prior exclusions to satisfy ExcluderType.");
  static_assert(ExcluderType<Exclusion>, "ExclusionChain requires appended exclusions to satisfy ExcluderType.");
 ...
 public:
  ...
  /// \brief Prepare every exclusion in the chain for the selected source and target chunks.
  ...
  void setup(const stk::mesh::BulkData& bulk_data, const stk::mesh::Selector& target_selector,
             const stk::mesh::Selector& source_selector) {
    prior_exclusions_.setup(bulk_data, target_selector, source_selector);
    exclusion_.setup(bulk_data, target_selector, source_selector);
  }
  //@}
  ...
  /// \tparam NextExclusion Excluder type to append.
  /// \param next_exclusion [in] Exclusion predicate to append to the chain.
  template <ExcluderType NextExclusion>
...
struct ExcludeSelfInteraction {
  ...
  /// \brief Prepare self-interaction exclusion for the selected source and target chunks.
  /// \param bulk_data [in] Unused bulk data.
  /// \param target_selector [in] Unused target selector.
  /// \param source_selector [in] Unused source selector.
  void setup(const stk::mesh::BulkData& /*bulk_data*/, const stk::mesh::Selector& /*target_selector*/,
...
template <typename ListType, typename ExecutionSpace,                            //
          ...
          ExcluderType Exclusions = NoExclusions>
class NeighborListBuilder {
 public:
  //! \name Aliases
  //@{
  ...
  NeighborListBuilder(const execution_space& exec_space, const target_input_type& target_input,
      ...
      : exec_space_(exec_space), targets_(target_input), sources_(source_input), exclusions_() {
  }
  ...
  template <typename Exclusion>
    requires ExcluderType<Exclusion>
  ...
  /// \brief Return a prepared copy of the exclusion chain.
  /// \param bulk_data [in] STK bulk data used for mesh-dependent excluder setup.
  exclusions_type setup_exclusions(const stk::mesh::BulkData& bulk_data) const {
    exclusions_type prepared_exclusions = exclusions_;
    prepared_exclusions.setup(bulk_data, target_selector(), source_selector());
    return prepared_exclusions;
  }
  //@}
  ...
  neighbor_list_type build(const stk::mesh::BulkData& bulk_data) const {
    const exclusions_type prepared_exclusions = setup_exclusions(bulk_data);
    (void)prepared_exclusions;
  ...
  /// `build(bulk_data)` or `setup_exclusions(bulk_data)`.
