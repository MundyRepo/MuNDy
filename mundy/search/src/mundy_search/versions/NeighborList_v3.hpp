...
// Forward declaration needed by NoExclusions::exclude().
...
class NoExclusions {
 public:
  ...
  /// \brief Return a new exclusion chain with one appended exclusion predicate.
...
class ExclusionChain {
 public:
  ...
  using prior_exclusions_type = PriorExclusions;
  using exclusion_type = Exclusion;
  //@}
  ...
  /// \brief Default constructor.
  KOKKOS_DEFAULTED_FUNCTION
  ExclusionChain() = default;

  /// \brief Construct a chain from a previous chain and a newly appended exclusion.
  /// \param prior_exclusions [in] Previous exclusion chain.
  /// \param exclusion [in] Newly appended exclusion predicate.
  KOKKOS_INLINE_FUNCTION
  ExclusionChain(const prior_exclusions_type& prior_exclusions, const exclusion_type& exclusion)
      : prior_exclusions_(prior_exclusions), exclusion_(exclusion) {
  }
  //@}
  ...
  /// \brief Return whether any exclusion in the chain rejects the candidate pair.
  /// \tparam Candidate Candidate pair type.
  /// \param candidate [in] Candidate pair produced by a search backend.
  template <typename Candidate>
  KOKKOS_INLINE_FUNCTION
  ...
  /// \brief Return a new exclusion chain with one additional appended exclusion predicate.
  ...
  /// \brief Get the previous exclusion chain.
  KOKKOS_INLINE_FUNCTION
  prior_exclusions_type prior_exclusions() const {
    return prior_exclusions_;
  }

  /// \brief Get the newly appended exclusion predicate.
  KOKKOS_INLINE_FUNCTION
  exclusion_type exclusion() const {
    return exclusion_;
  }
  //@}
 ...
 private:
  ...
  //! Previous exclusion chain.
  prior_exclusions_type prior_exclusions_;
  //! Newly appended exclusion predicate.
  exclusion_type exclusion_;
  //@}
};
...
class NeighborSearchCandidate {
 public:
  ...
  KOKKOS_INLINE_FUNCTION
  size_type target_owner_index() const noexcept {
    return target_index_;
  }

  /// \brief Get the dense source owner ordinal.
  KOKKOS_INLINE_FUNCTION
  size_type source_owner_index() const noexcept {
    return source_index_;
  }
  ...
  /// \brief Return whether the target and source have the same owner entity.
  KOKKOS_INLINE_FUNCTION
  bool is_same_owner() const noexcept {
    return target_entity_ == source_entity_;
  }

  /// \brief Return whether the relative image shift is zero.
  KOKKOS_INLINE_FUNCTION
  bool has_zero_relative_image_shift() const noexcept {
    return true;
  }
  //@}
...
class PeriodicNeighborSearchCandidate {
 public:
  ...
  /// \brief Return whether the relative image shift is exactly zero.
  KOKKOS_INLINE_FUNCTION
  bool has_zero_relative_image_shift() const noexcept {
    return relative_image_shift_[0] == static_cast<typename image_shift_type::scalar_t>(0) &&
           relative_image_shift_[1] == static_cast<typename image_shift_type::scalar_t>(0) &&
           relative_image_shift_[2] == static_cast<typename image_shift_type::scalar_t>(0);
  }
  //@}
...
class ArborXExclusionCallback {
 public:
  ...
  using exclusions_type = Exclusions;
  ...
  /// \brief Default constructor.
  KOKKOS_DEFAULTED_FUNCTION
  ArborXExclusionCallback() = default;

  /// \brief Construct from a candidate factory and an exclusion chain.
  /// \param candidate_factory [in] Candidate factory used to adapt ArborX hits.
  /// \param exclusions [in] Statically typed exclusion chain.
  KOKKOS_INLINE_FUNCTION
  ArborXExclusionCallback(const candidate_factory_type& candidate_factory, const exclusions_type& exclusions)
      : candidate_factory_(candidate_factory), exclusions_(exclusions) {
  }
  //@}
 ...
 private:
  ...
  //! Statically typed exclusion chain.
  exclusions_type exclusions_;
  //@}
};
...
/// \brief Builder-prepared exclusion that suppresses one orientation of symmetric target/source pairs.
///
...
class ExcludeSymmetricDuplicates {
 public:
  ...
  /// \brief Construct from bulk data and target/source selectors.
...
/// \brief Fluent, statically typed neighbor-list builder.
///
/// The builder owns construction policy, including selectors and exclusions. Each `.exclude(...)` call returns a new
/// builder type whose `exclusions_type` contains the complete concrete chain of previous exclusions plus the new one.
/// This is the intended extension point for half-list behavior, connected-entity exclusions, and user-defined rejection
/// rules.
/// \tparam NeighborListType Concrete neighbor-list type returned by `build()`.
/// \tparam ExecutionSpace Kokkos execution space used by the eventual build.
/// \tparam TargetSearchBoxes Target search-box wrapper type.
/// \tparam SourceSearchBoxes Source search-box wrapper type.
/// \tparam Exclusions Statically typed exclusion chain.
template <typename NeighborListType, typename ExecutionSpace, typename TargetSearchBoxes, typename SourceSearchBoxes,
          typename Exclusions = NoExclusions>
class NeighborListBuilder {
 public:
  //! \name Aliases
  //@{
  ...
  using target_search_boxes_type = TargetSearchBoxes;
  using source_search_boxes_type = SourceSearchBoxes;
  using exclusions_type = Exclusions;
  //@}
  ...
  /// \brief Construct a builder with no explicit selectors and the default empty exclusion chain.
  /// \param exec_space [in] Execution space used by the eventual build.
  /// \param targets [in] Target search boxes.
  /// \param sources [in] Source search boxes.
  NeighborListBuilder(const execution_space& exec_space, const target_search_boxes_type& targets,
                      const source_search_boxes_type& sources)
  ...
  /// \param targets [in] Target search boxes.
  /// \param sources [in] Source search boxes.
  /// \param target_selector [in] Host-side target selector.
  /// \param source_selector [in] Host-side source selector.
  /// \param exclusions [in] Statically typed exclusion chain.
  NeighborListBuilder(const execution_space& exec_space, const target_search_boxes_type& targets,
                      const source_search_boxes_type& sources, const stk::mesh::Selector& target_selector,
                      const stk::mesh::Selector& source_selector, const exclusions_type& exclusions)
      : exec_space_(exec_space),
        targets_(targets),
        sources_(sources),
  ...
  /// \brief Return a copy of this builder with an updated target selector.
  /// \param selector [in] Host-side target selector used by construction policy.
  NeighborListBuilder target_selector(const stk::mesh::Selector& selector) const {
    return NeighborListBuilder(exec_space_, targets_, sources_, selector, source_selector_, exclusions_);
  }

  /// \brief Return a copy of this builder with an updated source selector.
  /// \param selector [in] Host-side source selector used by construction policy.
  NeighborListBuilder source_selector(const stk::mesh::Selector& selector) const {
    return NeighborListBuilder(exec_space_, targets_, sources_, target_selector_, selector, exclusions_);
  }

  /// \brief Return a new builder type with an appended exclusion predicate.
  /// \tparam Exclusion Kokkos-callable exclusion predicate type.
  /// \param exclusion [in] Exclusion predicate to append.
  template <typename Exclusion>
  auto exclude(const Exclusion& exclusion) const {
    const auto new_exclusions = exclusions_.exclude(exclusion);
    using new_exclusions_type = decltype(new_exclusions);
    return NeighborListBuilder<neighbor_list_type, execution_space, target_search_boxes_type, source_search_boxes_type,
                               new_exclusions_type>(exec_space_, targets_, sources_, target_selector_, source_selector_,
                                                    new_exclusions);
  }

  /// \brief Return a new builder type with symmetric duplicate pair suppression appended.
  ///
  /// Uses the stored target and source selectors to determine the selector-overlap relationship and
  /// constructs the appropriate builder-prepared exclusion. All three cases (identical, disjoint,
  /// overlapping selectors) are handled correctly by impl::ExcludeSymmetricDuplicates.
  /// \param bulk_data [in] STK bulk data used to compute the selector intersection and per-bucket mask.
  auto exclude_symmetric_duplicates(const stk::mesh::BulkData& bulk_data) const {
    return exclude(impl::ExcludeSymmetricDuplicates(bulk_data, target_selector_, source_selector_));
  }
  //@}
  ...
  /// \brief Get the target search boxes.
  target_search_boxes_type targets() const noexcept {
    return targets_;
  }

  /// \brief Get the source search boxes.
  source_search_boxes_type sources() const noexcept {
    return sources_;
  }

  /// \brief Get the host-side target selector.
  const stk::mesh::Selector& target_selector() const noexcept {
    return target_selector_;
  }

  /// \brief Get the host-side source selector.
  const stk::mesh::Selector& source_selector() const noexcept {
    return source_selector_;
  }

  /// \brief Get the statically typed exclusion chain.
  exclusions_type exclusions() const noexcept {
    return exclusions_;
  }
  //@}
 ...
 private:
  ...
  //! Target search boxes.
  target_search_boxes_type targets_;
  //! Source search boxes.
  source_search_boxes_type sources_;
  //! Host-side target selector used by construction policy.
  stk::mesh::Selector target_selector_;
  //! Host-side source selector used by construction policy.
...
/// \brief Create a fluent neighbor-list builder seeded with an empty exclusion chain.
/// \tparam NeighborListType Concrete neighbor-list type returned by `build()`.
/// \tparam ExecutionSpace Kokkos execution space used by the eventual build.
/// \tparam TargetSearchBoxes Target search-box wrapper type.
/// \tparam SourceSearchBoxes Source search-box wrapper type.
