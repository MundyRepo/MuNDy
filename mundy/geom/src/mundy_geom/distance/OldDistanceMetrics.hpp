...
#ifndef MUNDY_GEOM_OLD_DISTANCE_METRICS_HPP_
#define MUNDY_GEOM_OLD_DISTANCE_METRICS_HPP_
...
KOKKOS_INLINE_FUNCTION Scalar old_pbc_floor(Scalar x) {
...
struct OldPeriodicSpaceMetric {
  ...
  Point<Scalar> operator()(const Point<Scalar>& point1, const Point<Scalar>& point2) const {
    ...
    for (size_t i = 0; i < 3; ++i) {
      point1_scaled[i] -= old_pbc_floor<int64_t, double>(point1_scaled[i]);
...
#endif  // MUNDY_GEOM_OLD_DISTANCE_METRICS_HPP_