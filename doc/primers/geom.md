...
See the {ref}`MundyGeom API directory <dir_mundy_core_geom>`.
...
MundyGeom builds directly on MundyMath. A {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>`\<Scalar\> behaves like a {ref}`Vector3 <exhale_class_classmundy_1_1AVector>`\<Scalar\>, so most geometry code is
ordinary vector arithmetic wrapped in shape-specific names. Just like MundyMath, the API is intentionally free-function
heavy: you construct light primitives, then call {ref}`distance <namespace_mundy>`, {ref}`compute_aabb <file_mundy_core_geom_src_mundy_geom_compute_aabb.hpp>`, `wrap_rigid`, {ref}`translate <file_mundy_core_geom_src_mundy_geom_transform.hpp>`, and similar
algorithms on them.
...
|----------|------------------|------------------------|
| {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>`\<Scalar\> | `x, y, z` | A position in 3D space. |
| {ref}`Line <exhale_class_classmundy_1_1Line>`\<Scalar\> | `center, direction` | An infinite oriented line. |
| {ref}`LineSegment <exhale_class_classmundy_1_1LineSegment>`\<Scalar\> | `start, end` | A finite segment between two points. |
| {ref}`VSegment <exhale_class_classmundy_1_1VSegment>`\<Scalar\> | `start, middle, end` | Two connected finite segments. |
| {ref}`AABB <exhale_class_classmundy_1_1AABB>`\<Scalar\> | `min_corner, max_corner` | An axis-aligned bounding box. |
| {ref}`Sphere <exhale_class_classmundy_1_1Sphere>`\<Scalar\> | `center, radius` | A ball with no orientation. |
| {ref}`Ellipsoid <exhale_class_classmundy_1_1Ellipsoid>`\<Scalar\> | `center, orientation, radii` | An oriented ellipsoid. |
| {ref}`Spherocylinder <exhale_class_classmundy_1_1Spherocylinder>`\<Scalar\> | `center, orientation, radius, length` | A capsule-like body aligned with its body z-axis. |
| {ref}`SpherocylinderSegment <exhale_class_classmundy_1_1SpherocylinderSegment>`\<Scalar\> | `start, end, radius` | A capsule-like body stored by endpoints. |
| {ref}`Circle3D <exhale_class_classmundy_1_1Circle3D>`\<Scalar\> | `center, orientation, radius` | An oriented circle in 3D. |
| {ref}`Ring <exhale_class_classmundy_1_1Ring>`\<Scalar\> | `center, orientation, major_radius, minor_radius` | An oriented thick ring. |

Oriented shapes use a {ref}`Quaternion <exhale_class_classmundy_1_1AQuaternion>`\<Scalar\>. The center is always in the lab frame, and the orientation maps the body's
...
For {ref}`AABB <exhale_class_classmundy_1_1AABB>`, `[]` treats the box as `{x_min, y_min, z_min, x_max, y_max, z_max}`.
...
### Views
Geom views come from MundyMath views. Since {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>` is an accessor-backed {ref}`Vector3 <exhale_class_classmundy_1_1AVector>`, and primitives are templated on
...
such as {ref}`translate <file_mundy_core_geom_src_mundy_geom_transform.hpp>`, generally return owning results.
...
| **Primitive** | **{ref}`reference_point(shape) <namespace_mundy>`** |
|---------------|-------------------------------|
| {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>` | the point itself |
| {ref}`Line <exhale_class_classmundy_1_1Line>` | `center()` |
| {ref}`LineSegment <exhale_class_classmundy_1_1LineSegment>` | `start()` |
| {ref}`VSegment <exhale_class_classmundy_1_1VSegment>` | `start()` |
| {ref}`AABB <exhale_class_classmundy_1_1AABB>` | `min_corner()` |
| {ref}`Sphere <exhale_class_classmundy_1_1Sphere>` | `center()` |
| {ref}`Ellipsoid <exhale_class_classmundy_1_1Ellipsoid>` | `center()` |
| {ref}`Spherocylinder <exhale_class_classmundy_1_1Spherocylinder>` | `center()` |
| {ref}`SpherocylinderSegment <exhale_class_classmundy_1_1SpherocylinderSegment>` | `start()` |
| {ref}`Circle3D <exhale_class_classmundy_1_1Circle3D>` | `center()` |
| {ref}`Ring <exhale_class_classmundy_1_1Ring>` | `center()` |
...
|---------|-------------|----------------|
| {ref}`SharedNormalSigned{} <exhale_struct_structmundy_1_1SharedNormalSigned>` | Signed separation measured along a shared normal. | Surface-based queries such as sphere-sphere or ellipsoid-ellipsoid. |
| {ref}`Euclidean{} <exhale_struct_structmundy_1_1Euclidean>` | Standard Euclidean distance. | Queries such as `Circle3D-Circle3D`. |
...
{ref}`compute_aabb <file_mundy_core_geom_src_mundy_geom_compute_aabb.hpp>` returns an axis-aligned box in the current coordinate frame. {ref}`compute_bounding_radius <file_mundy_core_geom_src_mundy_geom_compute_bounding_radius.hpp>` returns a scalar
...
|---------------|----------------------|-------------|
| `compute_aabb(shape)` | {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>`, {ref}`LineSegment <exhale_class_classmundy_1_1LineSegment>`, {ref}`Sphere <exhale_class_classmundy_1_1Sphere>`, {ref}`Ellipsoid <exhale_class_classmundy_1_1Ellipsoid>`, {ref}`Spherocylinder <exhale_class_classmundy_1_1Spherocylinder>`, {ref}`SpherocylinderSegment <exhale_class_classmundy_1_1SpherocylinderSegment>` | Conservative axis-aligned box in the current frame. |
...
| `compute_bounding_radius(shape)` | {ref}`Point <file_mundy_core_geom_src_mundy_geom_primitives_Point.hpp>`, {ref}`LineSegment <exhale_class_classmundy_1_1LineSegment>`, {ref}`Sphere <exhale_class_classmundy_1_1Sphere>`, {ref}`Ellipsoid <exhale_class_classmundy_1_1Ellipsoid>`, {ref}`Spherocylinder <exhale_class_classmundy_1_1Spherocylinder>`, {ref}`SpherocylinderSegment <exhale_class_classmundy_1_1SpherocylinderSegment>` | Conservative scalar radius about the geometric center. |
| `intersects(aabb0, aabb1)` | {ref}`AABB <exhale_class_classmundy_1_1AABB>` pairs | `true` when two AABBs overlap or touch. |
...
query such as {ref}`distance <namespace_mundy>` only when needed.
...
|------------|--------------------|
| {ref}`EuclideanMetric <exhale_class_classmundy_1_1EuclideanMetric>`\<Scalar\> | No periodic directions. |
| {ref}`PeriodicMetric <exhale_class_classmundy_1_1PeriodicMetric>`\<Scalar\> | Fully periodic general cell given by a cell matrix. |
| {ref}`PeriodicMetricX <exhale_class_classmundy_1_1PeriodicMetricX>`\<Scalar\> | Periodic only in x. |
| {ref}`PeriodicMetricY <exhale_class_classmundy_1_1PeriodicMetricY>`\<Scalar\> | Periodic only in y. |
| {ref}`PeriodicMetricXY <exhale_class_classmundy_1_1PeriodicMetricXY>`\<Scalar\> | Periodic in x and y. |
| {ref}`PeriodicMetricYZ <exhale_class_classmundy_1_1PeriodicMetricYZ>`\<Scalar\> | Periodic in y and z. |
| {ref}`PeriodicScaledMetric <exhale_class_classmundy_1_1PeriodicScaledMetric>`\<Scalar\> | Axis-aligned periodic box given by side lengths. |
...
Rotation is defined for all common primitives except {ref}`AABB <exhale_class_classmundy_1_1AABB>`. Rotating an {ref}`AABB <exhale_class_classmundy_1_1AABB>` is intentionally not supported because
...
| **Generator** | **Meaning** |
|---------------|-------------|
| `generate_random_point<Scalar>(box, rng)` | Uniform point in an {ref}`AABB <exhale_class_classmundy_1_1AABB>`. |
